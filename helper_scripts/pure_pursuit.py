...
class PurePursuit:
    ...
    def timer_callback(self, event):
        ...
        )  # np.dot(np.linalg.inv(map_T_robot), np.array([goal[0], goal[1],1])) #
        goal = goal[0:2]
        ##### YOUR CODE ENDS HERE #####
        ...
        final_goal.header = header
        final_goal.pose.position.x = relative_goal[0]
        final_goal.pose.position.y = relative_goal[1]
        # final_goal.z = yaw
        if not np.isnan(final_goal.pose.position.x) and not np.isnan(
            final_goal.pose.position.y
        ):  # ensure data is valid
            self.final_goal_pub.publish(final_goal)
            print("Final goal", relative_goal[0], relative_goal[1])
